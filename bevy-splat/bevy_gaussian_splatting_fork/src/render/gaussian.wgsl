...
#import bevy_gaussian_splatting::entrance_animation::assemble
...
fn vs_points(
    ...
    var raw_position = get_position(splat_index);

    // Apply entrance animation when time > 0 and < animation duration
    // Animation uses time uniform, speed=2.0, depth_offset=10.0
    let anim_duration = 25.0;
    if (gaussian_uniforms.time > 0.0 && gaussian_uniforms.time < anim_duration) {
        // Get scale - use default if PRECOMPUTE_COVARIANCE_3D is enabled
        #ifdef PRECOMPUTE_COVARIANCE_3D
            let scale = vec3<f32>(1.0, 1.0, 1.0);
        #else
            let scale = get_scale(splat_index);
        #endif
        let anim_result = assemble(raw_position, scale, gaussian_uniforms.time, 10.0, 2.0);
        raw_position = anim_result.position;

        // Early discard if particle not yet visible
        if (anim_result.opacity < 0.01) {
    ...
    let position = vec4<f32>(raw_position, 1.0);
